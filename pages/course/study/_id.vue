  ...
  methods: {
    ...
    getPlayer(vid, playAuth) {
      ...
      this.player = new Aliplayer({
        ...
        // 创建完成之后获取上次播放进度
        userVideo.getLastTime(this.videoId)
          .then(response => {
    ...
    recordTime() {
      ...
      var timer = setInterval(() => {
        ...
        if (this.player && this.player.getStatus() === 'playing') {
          ...
          userVideo.lastVideoTime(this.videoId, curTime)
            .then(response => {
            })
        }
      }, 10000)
    ...
    onPlay() {
      var onPlayEvent = () => {
        ...
        userVideo.recordAllTime(this.videoId, this.allTime)
          .then(response => {
          })
      }
    ...
    onEnd() {
      ...
      var onEndEvent = () => {
        ...
        userVideo.accomplishVideo(this.courseId, this.videoId)
          .then(response => {
          })
        // 播放完毕清零
        this.allTime = 0
