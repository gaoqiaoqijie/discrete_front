      ...
      <div class="i-box">
        ...
        <div v-if="isUserInfo">
          ...
          <el-tabs tab-position="left" style="height: 100%" @tab-click="handleClick">
            <el-tab-pane label="基本资料修改" @click="getUserInfo">
...
<script>
import userCourse from '@/api/learning/userCourse'
import userVideo from '@/api/learning/userVideo'
...
export default {
  ...
  methods: {
    ...
    // 获取我参与的课程
    getMyJoinCourse() {
      userCourse.getMyJoinCourse(this.page, this.limit)
        .then(response => {
    ...
    getMyLearningTime() {
      console.log(this.userInfo)
      userVideo.getLearningTime(this.userInfo.id)
        .then(response => {
    ...
    // 更新用户信息
    updateUserInfo() {
      user.updateUserInfo(this.userInfo)
        .then(response => {
    ...
    updatePassword() {
      ...
      user.updatePassword(this.passwordVo)
        .then(response => {
          if (response.data.code === 200) {
            this.$message({
              type: 'success',
    ...
    // 课程列表切换
    gotoCoursePage(page) {
      userCourse.getMyJoinCourse(page, this.limit)
        .then(response => {
          this.courseData = response.data.data.rows
          this.total = response.data.data.total
        })
