  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogVideoFormVisible" title="添加小节">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-loading.fullscreen="fullscreenLoading" :disabled="videoUploadSucess" type="primary" @click="saveOrUpdateVideo">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import vod from '@/api/learning/vod.js'
import request from '@/utils/request'

export default {
  data() {
    return {
      videoUploadSucess: true, // 视频上传成功
  ...
  methods: {
    ...
    getNestedTreeList() {
      ...
      chapter.getTreeList(this.courseId)
        .then(response => {
          this.chapterNestedList = response.data.data.treeChapter
        })
    },
    ...
    fetchVideo(videoId) {
      ...
      video.getVideoInfo(videoId)
        .then(response => {
          this.videoForm = response.data.data.video
          // 回显视频名字信息
          this.fileList = [{ 'name': response.data.data.video.videoOriginalName }]
        })
    },
    // 更新小节数据方法
    updateVideo() {
    ...
    // 上一步
    previous() {
      this.$router.push({ name: 'add', params: { id: this.courseId }})
    },
    // 下一步
    next() {
      this.$router.push({ name: 'add-publish', params: { id: this.courseId }})
    }
  }
}
</script>
